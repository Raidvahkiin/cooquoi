...
export abstract class Unit extends Enumeration {
	...
	static castTo(type: string) {
		let getValueFn: (value: string) => Unit;

		switch (type) {
			case WeightUnit.name:
				getValueFn = WeightUnit.fromValue;
				break;
			case VolumeUnit.name:
				getValueFn = VolumeUnit.fromValue;
				break;
			case PieceUnit.name:
				getValueFn = PieceUnit.fromValue;
				break;
			default:
		...
		return {
			fromValue: (value: string): Unit => getValueFn(value),
		};
	}
}
